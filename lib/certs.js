...
const { normalizeDomain, generateKey, parseCertificate, validationErrors, isRenewalDue, renewalThreshold } = require('./tools');
...
class Certs {
    ...
    constructor(options) {
        ...
        this.acme = ACME.create({
            ...
            __request: createAcmeRequest(this.dispatcher),
            notify: (ev, params) => {
    ...
    async acquireCert(domain) {
        ...
        if (existingCertificateData && !isRenewalDue(existingCertificateData)) {
            // no need to renew
            return existingCertificateData;
        }
        ...
        try {
            ...
            this.logger.info({
                ...
                renewAfter: new Date(parsed.validTo.getTime() - renewalThreshold(parsed))
            });
...
module.exports = { Certs, isRenewalDue, renewalThreshold };
